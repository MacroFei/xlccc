...
<head>
    ...
    <script src="crops_js/selectCrops.js"></script>
    <script type="text/javascript">
        $().ready(function(){
            checkLogin();
            selectSheng();
    ...
    <div class="mainwrapper">
        ...
        <div class="wrapper" style="min-height: 582px;">
            ...
            <table id="tb" class="table table-bordered table-striped" style="width: 90%;margin-left: 5%;">
                <tr>
                    <td>
                        <input type="checkbox" onclick="allChecked(this)" value="${c.id}" class="inp"/>
                    </td>
                    <td>ID</td>
                    <td>品种</td>
                    <td>温度</td>
                ...
                <form action="crop_deleteAll" method="post">
                    <c:forEach items="${cs}" var="c">
                        <tr>
                            <td>
                                <input type="checkbox" id="checkbox" name="checkbox" value="${c.id}" class="inp"/>
                            </td>
                            <td>${c.id} </td>
                            <td>${c.variety}</td>
                            <td>${c.temperayure}℃</td>
                            <td>${c.humidity}％</td>
                            <td>${c.co2} ìl/L</td>
                            <td>${c.illumination}lx</td>
                            <td>${c.province} ${c.city} ${c.district}</td>
                            <td>
                                <a href="#" class="big-link" data-reveal-id="${c.id}" data-animation="fade">
                                    <button class="btn btn-success" onclick="location.href='#'">查看</button>
                                </a>
                                <div id="${c.id}" class="reveal-modal">
                                    <h4 style="float: left;">查看信息</h4>
                                    <br>
                                    <br>
                                    <p>品种：${c.variety} </p>
                                    <p>温度：${c.temperayure} ℃</p>
                                    <p>湿度：${c.humidity} %</p>
                                    <p>CO2：${c.co2} ìl/L</p>
                                    <p>光照：${c.illumination} lx</p>
                                    <p>所属区域：${c.province} ${c.city} ${c.district} </p>
